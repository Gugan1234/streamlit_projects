...
class Snake:
    def __init__(self):
        ...
        for i in range(0, BODY_PARTS):
            self.coordiates.append([0,0])
        for x,y in self.coordiates:
            square = canvas.create_rectangle(x,y,x+SPACE,y+SPACE,fill=SNAKE_COLOR,tag="snake")
            self.squares.append(square)

class Food:
    def __init__(self):
        x = random.randint(0,13) * SPACE
        y = random.randint(0, 13) * SPACE
...
def check_collision(snake):
    ...
    if x<0 or x>=GAME_WIDTH:
        ...
        print("GAME OVER")
        return True
    for body_part in snake.coordiates[1:]:
